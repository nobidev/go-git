...
// sharedFile provides shared, reference-counted access to a file that
// is opened lazily and closed automatically when no readers remain.
//
// Multiple goroutines can acquire the file concurrently; they all
// share a single underlying file descriptor. The FD is opened on
// first acquire and closed after a grace period once the last active
...
// cannot be deleted — while still sharing a single FD across
// concurrent readers and avoiding repeated open/close syscalls for
// sequential operations.
//
// The opener MUST return a billy.File whose ReadAt is concurrent-safe
// across goroutines (the documented contract on billy.File.ReadAt as
// of go-billy main). sharedFile does not synchronise concurrent
// ReadAt calls itself.
//
// All methods are safe for concurrent use.
type sharedFile struct {
	opener      func() (billy.File, error)
...
// sharedFile. Wrapped by PackHandle.OpenPackReader for the same case.
var errSharedFileClosed = errors.New("packhandle: shared file is closed")

// newSharedFile constructs a sharedFile that lazily opens its
// underlying handle via opener. The opener is invoked on first
// acquire and again after each grace-period close.
func newSharedFile(opener func() (billy.File, error)) *sharedFile {
...
// prevents the file descriptor from ever closing. Returns
...
// the sharedFile has been permanently closed or grace is zero).
...
// closeLocked closes the underlying file. Must be called with mu
// held. Also clears any pending close timer.
