...
// trackingFile is a billy.File backed by a bytes.Reader that tracks
// Close. All write-side methods return os.ErrPermission to match the
// read-only contract sharedFile uses.
type trackingFile struct {
	*bytes.Reader
	closed atomic.Bool
}

func (f *trackingFile) Close() error                          { f.closed.Store(true); return nil }
func (f *trackingFile) Name() string                          { return "tracking" }
func (f *trackingFile) Write([]byte) (int, error)             { return 0, os.ErrPermission }
func (f *trackingFile) WriteAt([]byte, int64) (int, error)    { return 0, os.ErrPermission }
func (f *trackingFile) Truncate(int64) error                  { return os.ErrPermission }
func (f *trackingFile) Stat() (fs.FileInfo, error)            { return nil, errors.New("not implemented") }

var _ billy.File = (*trackingFile)(nil)

// newTestSharedFile creates a sharedFile with zero grace period for
// deterministic test behaviour.
func newTestSharedFile(opener func() (billy.File, error)) *sharedFile {
...
func TestSharedFileGracePeriodResetByAcquire(t *testing.T) {
	...
	// After the new grace period expires: file closed. Poll instead
	// of sleeping a fixed amount so we don't race timer scheduling
	// jitter.
