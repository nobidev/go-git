...
func (d *Decoder) readExtension(idx *Index) error {
	...
	trace.Internal.Printf("index: decode extension header %q", header)
	...
	case bytes.Equal(header[:], endOfIndexEntryExtSignature):
		...
		trace.Internal.Printf("index: end-of-index-entry extension decoded, offset %d hash %s", idx.EndOfIndexEntry.Offset, idx.EndOfIndexEntry.Hash.String())
	default:
		// See https://git-scm.com/docs/index-format, which says:
		// If the first byte is 'A'..'Z' the extension is optional and can be ignored.
		if header[0] < 'A' || header[0] > 'Z' {
			trace.Internal.Printf("index: unknown mandatory extension %q", header)
			return ErrUnknownExtension
		}

		trace.Internal.Printf("index: skipping optional unknown extension %q", header)
...
func (d *Decoder) readChecksum(expected []byte) error {
	...
	if h.Compare(expected) != 0 {
		trace.Internal.Printf("index: checksum mismatch, expected %x got %x", expected, h)
		return ErrInvalidChecksum
	}

	trace.Internal.Printf("index: checksum ok %s", h.String())
	return nil
}
