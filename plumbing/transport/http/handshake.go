...
func describeInfoRefsError(err error, resp *http.Response, req *transport.Request, head []byte) error {
	// Name the URL actually fetched, which after a redirect is not req.URL.
	fetched := req.URL
	...
	if mediaType == "text/plain" {
		return fmt.Errorf("%w: %s served content type %q: %w: %q",
			...
			strings.TrimSpace(string(trimPartialRune(head))))
	}
...
func handshakeDumb(resp *http.Response, req *transport.Request, client *http.Client, authorizer func(*http.Request) error) (transport.Session, error) {
	...
	if err := infoRefs.Decode(rd); err != nil {
		return nil, describeInfoRefsError(err, resp, req, head)
	}
