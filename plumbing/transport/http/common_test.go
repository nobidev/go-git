...
func (s *ClientSuite) TestCheckRedirectBlocksDowngradeBeforeTheHop(c *C) {
	...
	secureBase = probe.serve(c, "example.test", true, func(w http.ResponseWriter, req *http.Request) {
		...
		_, err := w.Write([]byte(uploadPackAdvertisement()))
		c.Assert(err, IsNil)
	})
	...
	c.Assert(err, ErrorMatches, ".*changes scheme from \"https\" to \"http\".*")

	probe.mu.Lock()
	defer probe.mu.Unlock()
	c.Assert(probe.dialed, HasLen, 0)
	c.Assert(probe.plainAuth, HasLen, 0)
}
